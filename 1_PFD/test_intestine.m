...
parsedGPR = GPRparser_xl(IntestineModel);% Extracting GPR data from model
IntestineModel.parsedGPR = parsedGPR;
% update the epsilon list
epsilon_new_f = 0.01 * ones(length(IntestineModel.rxns),1);
epsilon_new_r = epsilon_new_f;
for i = 1: length(IntestineModel.rxns)
    if any(ismember(IntestineModel.rxns(i), model.rxns))
        epsilon_new_f(i) = epsilon_f(ismember(model.rxns,IntestineModel.rxns(i)));
        epsilon_new_r(i) = epsilon_r(ismember(model.rxns,IntestineModel.rxns(i)));
    end
end
% integration 
