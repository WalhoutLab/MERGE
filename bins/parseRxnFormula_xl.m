function [metaboliteList, stoichCoeffList, revFlag] = parseRxnFormula_xl(formula)
...
for i = 1:length(tokens)
    ...
    elseif strcmp(t,'<=>') || strcmp(t,'<==>')
        ...
        if strcmp(t(1),'(') % is iCEL coeff
            sCoeff = str2double(t(2:end-1));
            % Stoich coefficient
            if ~productFlag
                sCoeff = -sCoeff;
            end
            ...
            % Metabolite name
            metaboliteList{end+1} = [t(2:end),'[',t(1),']'];
            if newMetFlag
                if ~productFlag
