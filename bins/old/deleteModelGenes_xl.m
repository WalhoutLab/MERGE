function [model, hasEffect, constrRxnNames, deletedGenes] = deleteModelGenes_xl(model, geneList, downRegFraction)
...
if (all(isInModel))
  ...
  model.genes(geneInd) = strcat(model.genes(geneInd),'_deleted');
    ...
    if (~isempty(rxnInd))
        ...
        % change the rxn state anyway
        for j = 1:length(rxnInd)
            if (isfield(model, 'rules') && ~isempty(model.rules{rxnInd(j)})) %To avoid errors if the rule is empty
                if (~eval(model.rules{rxnInd(j)})) %instead of ~eval(model.rules{rxnInd(j)})
                    constrainRxn(j) = true;
                end
            end
        end
        %constrain the solo OR anyway
        if length(rxnInd) == 1
            if eval(model.rules{rxnInd(1)})
            	constrainRxn(j) = true;
            end
        end
