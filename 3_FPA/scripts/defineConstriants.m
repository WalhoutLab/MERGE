...
model.lb(ismember(model.rxns,vitamins)) = -infDefault;%artificially set as infDefault
% set the maintaince
model = changeRxnBounds(model,'DM_atp_c_','l',0); %remove ATP coupling

% for FPA, we allow unlimited nutrient as long as it is avaible in the
% media (because flux scale is controled by flux allowance)
