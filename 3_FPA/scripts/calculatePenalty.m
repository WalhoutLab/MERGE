...
% rules. In brief, we take the "sum" of all "OR"-gated genes in a single
...
% detailed explanation, see supplemental information of the paper. 
% 
% USAGE:
%
%    penalty = calculatePenalty(model,master_expression,manualPenalty)
%
% NOTE: penalty caluclation uses `parfor` by default. Please change the 
% parfor loop to for loop if parfor is not supported in your environment.
%
% INPUTS:
...
%                       This input will overide all automatic calculated   
%                       penalty from expression data.
%
%
% OUTPUT:
...
for i = 1:length(model.rxns)
    ...
    if any(lenL ~= lenL(1)) % some GPR parsing error
        ...
        % then, pick the minimal value of all blocks (because they are
        % "AND"-connected)
...
% add a super condition column at the end of the penalty matrix
