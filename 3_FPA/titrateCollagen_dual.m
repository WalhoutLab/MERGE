...
parfor i = 1:length(nSeq)
    ...
    n = nSeq(i);
    %% demand reaction
    if contains(targetExRxns{1},'DMN') || contains(targetExRxns{1},'SNK') %demand reaction; note in the dual model, sink is actually demand, the uptake side is replaced by UPK
        ...
        % demand reaction
        myMet = setdiff(regexprep(model.mets(model.S(:,strcmp(model.rxns,[targetExRxns{1},'_X'])) < 0),'_.\[.\]',''),helperMet);
        ...
        [fluxEfficiency_X_met(i,:),fluxEfficiency_plus_X_met(i,:)] = FPA(model_tmp,{[targetExRxns{1},'_X']},master_expression,distMat,labels,n, manualPenalty2,manualDist,maxDist,blockList,constantPenalty2,false,penalty_defined_X);
        model_tmp = model;
        ...
        [fluxEfficiency_I_met(i,:),fluxEfficiency_plus_I_met(i,:)] = FPA(model_tmp,{[targetExRxns{1},'_I']},master_expression,distMat,labels,n, manualPenalty,manualDist,maxDist,blockList_I,constantPenalty,false,penalty_defined_I);
    %% sink or uptake reaction
    elseif contains(targetExRxns{1},'UPK') %sink or uptake reaction
        ...
        % in question
        manualDist_tmp_X = manualDist(~strcmp(manualDist(:,1),[targetExRxns{1},'_X_r']),:);
        manualDist_tmp_I = manualDist(~strcmp(manualDist(:,1),[targetExRxns{1},'_I_r']),:);
        ...
        model_tmp.ub(ismember(model_tmp.rxns,myRxns_X)) = 0;
        model_tmp.lb(ismember(model_tmp.rxns,myRxns_X)) = 0;
        [fluxEfficiency_X_met(i,:),fluxEfficiency_plus_X_met(i,:)] = FPA(model_tmp,{[targetExRxns{1},'_X']},master_expression,distMat,labels,n, manualPenalty2,manualDist_tmp_X,maxDist,blockList,constantPenalty2,false,penalty_defined_X);
        model_tmp = model;
        myRxns_I = myRxns(cellfun(@(x) ~isempty(regexp(x,'(_L|_I)$','once')),myRxns));
        model_tmp.ub(ismember(model_tmp.rxns,myRxns_I)) = 0;
        model_tmp.lb(ismember(model_tmp.rxns,myRxns_I)) = 0;
        [fluxEfficiency_I_met(i,:),fluxEfficiency_plus_I_met(i,:)] = FPA(model_tmp,{[targetExRxns{1},'_I']},master_expression,distMat,labels,n, manualPenalty,manualDist_tmp_I,maxDist,blockList_I,constantPenalty,false,penalty_defined_I);
    %% transporter reactions
    elseif contains(targetExRxns{1},'TCE') %transporter reactions
        ...
        myMetSet = setdiff(regexprep(model.mets(model.S(:,strcmp(model.rxns,[targetExRxns{1},'_X']))<0),'_.\[.\]',''),helperMet);
        ...
        model_tmp = model;
        targetrxn_fullName = [targetExRxns{1},'_X'];
        ...
        % extracellular side
        if any(strcmp([targetExRxns{1},'_L'],model.rxns)) % lumen transporter exists
            if length(myMetSet)==1 %only one metabolite is tranported, so we know the target met
                %check if it is being imported
                if ismember({[myMetSet{1},'_I[c]']},model.mets(model.S(:,strcmp(model.rxns,[targetExRxns{1},'_L']))>0)) %if the right side of the reaction is the [c] labeled met
                    targetrxn_fullName = [targetExRxns{1},'_L']; %use the lumen side for the forward direction
                else
                    targetrxn_fullName = [targetExRxns{1},'_I']; %use the EX side for the forward direction (secreting)
                end
            elseif isempty(myMetSet) %one or more minor met is involved. now see if the target met can be determined
                if length(regexprep(model.mets(model.S(:,strcmp(model.rxns,[targetExRxns{1},'_X']))<0),'_.\[.\]','')) == 1 %only one involved metabolite
                    tmpMet = regexprep(model.mets(model.S(:,strcmp(model.rxns,[targetExRxns{1},'_X']))<0),'_.\[.\]','');
                    %check if it is being imported
                    if ismember({[tmpMet{:},'_I[c]']},model.mets(model.S(:,strcmp(model.rxns,[targetExRxns{1},'_L']))>0)) %if the right side of the reaction is the [c] labeled met
                        targetrxn_fullName = [targetExRxns{1},'_L']; %use the lumen side for the forward direction
                    else
                        targetrxn_fullName = [targetExRxns{1},'_I']; %use the EX side for the forward direction (secreting)
                    end
                else
                    targetrxn_fullName = [targetExRxns{1},'_I'];%cannot decide, use EX side
                end
            else
                targetrxn_fullName = [targetExRxns{1},'_I']; %use the EX side for undetermined met target
            end
        else
            targetrxn_fullName = [targetExRxns{1},'_I'];%still calculate it even though it should be zero
        end
        model_tmp = model;
        for k = 1:length(myMetSet) %will be skipped if no myMet was found
            myMet = myMetSet{k};
        ...
        if any(strcmp([targetExRxns{1},'_L'],model.rxns)) % lumen transporter exists
            if length(myMetSet)==1 %only one metabolite is tranported, so we know the target met
                %check if it is being imported
                if ismember({[myMetSet{1},'_I[c]']},model.mets(model.S(:,strcmp(model.rxns,[targetExRxns{1},'_L']))<0)) %if the left side of the reaction is the [c] labeled met
                    targetrxn_fullName = [targetExRxns{1},'_L']; %use the lumen side for the reverse direction
                else
                    targetrxn_fullName = [targetExRxns{1},'_I']; %use the EX side for the reverse direction (secreting)
                end
            elseif isempty(myMetSet) %one or more minor met is involved. now see if the target met can be determined
                if length(regexprep(model.mets(model.S(:,strcmp(model.rxns,[targetExRxns{1},'_X']))<0),'_.\[.\]','')) == 1 %only one involved metabolite
                    tmpMet = regexprep(model.mets(model.S(:,strcmp(model.rxns,[targetExRxns{1},'_X']))<0),'_.\[.\]','');
            ...
            targetrxn_fullName = [targetExRxns{1},'_I'];%default
        end
        model_tmp = model;
        for k = 1:length(myMetSet) %will be skipped if no myMet was found
            myMet = myMetSet{k};
    ...
    elseif contains(targetExRxns{1},'NewMet_') %new reaction needed
