%% This is to reproduce the tissue FPA results in supp table S7 and S8
...
% here we reproduce the reactions in table S7 and S8
...
% This will grant some intestinal rxns to have identical penalty across all tissue optimizations.
...
% To get the constant penalty for intestine, we cheat the program by an expression 
% profile mimic where the genes are labels with "_I" suffix.
master_expression2 = {};
for i = 1:length(tissueLabel)
    expression = struct();
...
%% PART II: do the metabolite-centered FPA
% the difference (from rxn-centered) is that specific rxn(s) need to be blocked to prevent loop and
% false FPA value (see supp text for details). In addition, we need to optimize the
% forward and/or reverse direction of a transporter seperately.
% We are mainly interested in the transportation of organic molecules. So, some
% co-transporting metabolite are not the target of interest so that they shouldnt be blocked (limiting)
...
parfor i = 1:length(targetExRxns)
    ...
    if contains(targetExRxns{i},'DMN') || contains(targetExRxns{i},'SNK') %demand reaction; note in the dual model, sink is actually demand, the uptake side is replaced by UPK
        ...
        % and only block the reactions for corresponding tissue
        model_tmp = model;
